...
echo "openssl req -new -x509 -config openssl.conf -key ca.key -out ca.crt"
openssl req -new -x509 -config openssl.conf -key ca.key -out ca.crt
...
echo "openssl req -new -config openssl.conf -key client.key -out client.csr"
...
# csr for server
echo "openssl req -new -config openssl.conf -key server.key -out server.csr"
openssl req -new -config openssl.conf -key server.key -out server.csr
# sign csr of server with ca
echo "openssl x509 -req -config openssl.conf -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out server.crt"
